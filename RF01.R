...
wbcd <- wbcd[-1]
View(wbcd)
wbcd$diagnosis <- factor(wbcd$diagnosis, levels = c("B","M"), labels = c("Beningn","Malignant"))
wbcd$diagnosis
...
#wbcd$diagnosis <- factor(wbcd$diagnosis, levels = c("B","M"), labels = c("B","M"))
#wbcd$diagnosis
...
# Building a random forest model on training data 
wbcd_forest <- randomForest(diagnosis~.,data=wbcd_n,importance=TRUE, ntree=500)
??randomForest
plot(wbcd_forest)
